...
/*
  ...
  Note to SELF
     ...
     **     Copy_List was changed to Copy_list () const
            to accommodate the copy ctor for poly which has
     ...
     ****   insert function where previously if there were two same
            ...
            changed so that if it occurs the two items are added
            together.example
    ...
    Features:
        ...
        -Implemented:
            ...
            which is useful when verifying resulting poly in user interface
    ...
    Reflections:
        Had trouble every now and then on some syntax but it was 100% easier 
        project that the first time I had to do this project with arrays.

        For Extraction Operator I wanted the user to type in the entire 
        polynomial in a single line so I used a stringstream to change the input
        string to a bunch of strings and extract the coefficient and the power.
        Only problem with this was user has to type in four things to make up a 
        poly(coef,x,^ and power) Input must always look like 3x^4  or +3x^4 or 
        -3x^4
